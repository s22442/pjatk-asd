...
def print_sorting_time(label: str, sort_cb: callable(int)):
    ...
    print(
        f" - sorted samples: {timeit(lambda: sort_cb(random_samples), number = TIMEIT_NUMBER):.6f}"
    )
    print(
        f" - reversed sorted samples: {timeit(lambda: sort_cb(random_samples), number = TIMEIT_NUMBER):.6f}"
    )
